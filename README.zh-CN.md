...
最该避免的错误是每家厂商写一套实现。内置的 catalog 里有 **191 家 provider、6237 个模型，
...
Sources/AIKit/
  ...
  Providers/   catalog            （191 个 JSON 配置，纯数据）
  Tokens/      上下文分摊
  Client/      把它们连起来的管道
```
...
早期，API 会变。五种协议的流式响应和请求编码都能用了，catalog 覆盖 191 家。
...
| Provider catalog | ✅ 191 家、6237 模型 |
| 思考开 / 关 / 分级 | ✅ 全协议 |
